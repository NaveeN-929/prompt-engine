#!/usr/bin/env python3
"""
Docker-specific setup script for Ollama models - Autonomous Agent
"""
...
def main():
    """Main setup function for Docker-based Ollama - Autonomous Agent"""
    print("🤖 Autonomous Agent Docker Ollama Model Setup")
    print("=" * 50)
    ...
    # Define required models for Autonomous Agent
    required_models = ["llama3.1:8b"]  # Primary model for autonomous agent
    alternative_models = ["mistral:latest", "llama3.2:3b", "phi3:3.8b"]
    ...
    for model in required_models:
        if model in final_models:
            print(f"✅ Autonomous Agent model ready: {model}")
            agent_ready = True
            break
    ...
    if agent_ready:
        ...
        print("\nNext steps:")
        print("1. Start autonomous agent: python run_agent.py")
        print("2. Test agent: curl http://localhost:8000/agent/status")
        print("3. Check integration: curl http://localhost:8000/analyze")
        return True
    else:
        print("\n❌ Model setup failed")
